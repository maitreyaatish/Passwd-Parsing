...
let a = parse(fs.readFileSync('/etc/passwd', 'utf8'))  //Parsing passwd file
let b = parse(fs.readFileSync('/etc/group', 'utf8'))  //Parsing group file with the same module for simplicity of the code. The key's will be different in the output
a.forEach(function(n){ // Creating intermediate subarrays and pusing them into the main array that will be returned
  let temp = {}
  temp['uid'] = n['uid']
...
b.forEach(function(n){ //Getting groups by matching group id's with those form the passwd file
